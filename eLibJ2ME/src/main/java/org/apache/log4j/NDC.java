 ...
 * <p>
 * <li>When entering a context, call <code>NDC.push</code>. As a side effect, if there is no nested
 * diagnostic context for the current thread, this method will create it.
 * <p>
 ...
 * </ul>
 * <p>
 * There is no penalty for forgetting to match each <code>push</code> operation with a corresponding
 * <code>pop</code>, except the obvious mismatch between the real application context and the
 * context set in the NDC.
 * <p>
 * If configured to do so, {@link PatternLayout} and {@link TTCCLayout} instances automatically
 * retrieve the nested diagnostic context for the current thread without any user intervention.
 * Hence, even if a servlet is serving multiple clients simultaneously, the logs emanating from the
 * same code (belonging to the same category) can still be distinguished because each client request
 * will have a different NDC tag.
 * <p>
 * Heavy duty systems should call the {@link #remove} method when leaving the run method of a
 * thread. This ensures that the memory used by the thread can be freed by the Java garbage
 * collector. There is a mechanism to lazily remove references to dead threads. In practice, this
 * means that you can be a little sloppy and sometimes forget to call {@link #remove} before exiting
 * a thread.
 * <p>
...
public class NDC {
   ...
   * <p>
   * Internally a diagnostic context is represented as a stack. A given thread can supply the stack
   * (i.e. diagnostic context) to a child thread so that the child can inherit the parent thread's
   * diagnostic context.
   * <p>
   ...
   * {@link #cloneStack} method. It should communicate this information to its child so that it may
   ...
   * <p>
   * Each thread that created a diagnostic context by calling {@link #push} should call this method
   * before exiting. Otherwise, the memory used by the <b>thread</b> cannot be reclaimed by the VM.
   * <p>
   * As this is such an important problem in heavy duty systems and because it is difficult to
   * always guarantee that the remove method is called before exiting a thread, this method has been
   * augmented to lazily remove references to dead threads. In practice, this means that you can be
   * a little sloppy and occasionally forget to call {@link #remove} before exiting a thread.
   * However, you must call <code>remove</code> sometime. If you never call it, then your
   ...
   * the case that at the end of complex call sequences, the depth of the NDC is unpredictable. The
