 ...
 * <p>
 * It is sometimes useful to see how log4j is reading configuration files. You can enable log4j
 * internal logging by defining the <b>log4j.debug </b> variable.
 * <P>
 * At the initialization of the Category class, the file <b>log4j.properties </b> will be searched
 * from the search path used to load classes. If the file can be found, then it will be fed to the
...
public class PropertyConfigurator {
   ...
   *
   * <p>
   * This syntax means that one of the strings values ERROR, WARN, INFO or DEBUG can be supplied
   * followed by appender names separated by commas.
   * <p>
   * If one of the optional priority values ERROR, WARN, INFO or DEBUG is given, the root priority
   * is set to the corresponding priority. If no priority value is specified, then the root priority
   * remains untouched.
   * <p>
   * The root category can be assigned multiple appenders.
   * <p>
   * Each <i>appenderName </i> (seperated by commas) will be added to the root category. The named
   ...
   *
   * <p>
   * Thus, one of the usual priority values FATAL, ERROR, WARN, INFO, or DEBUG can be optionally
   * specified. For any any of these values the named category is assigned the corresponding
   * priority. In addition however, the value INHERITED can be optionally specified which means that
   * named category should inherit its priority from the category hierarchy.
   * <p>
   * If no priority value is supplied, then the priority of the named category remains untouched.
   * <p>
   * By default categories inherit their priority from the hierarchy. However, if you set the
   * priority of a category and later decide that that category should inherit its priority, then
   * you should specify INHERITED as the value for the priority value.
   * <p>
   * Similar to the root category syntax, each <i>appenderName </i> (seperated by commas) will be
   * attached to the named category.
   * <p>
   * See the <a href="../../manual.html#additivity">appender additivity rule </a> in the user manual
   * for the meaning of the <code>additivity</code> flag.
   * <h3>Example</h3>
   * <p>
   * An example configuration is given below. Other configuration file examples are given in
   * {@link org.apache.log4j.examples.Sort}class documentation.
   *
   * <pre>
   *
   *
  ...
  void configureOptionHandler(final OptionHandler oh, final String prefix, final Properties props) {
    ...
    for (final String option : options) {
      value = props.getProperty(prefix + option);
      LogLog.debug("Option " + option + "=[" + (value == null ? "null" : value) + "].");
      // Some option handlers assume that null value are not passed to
      // them.
      // So don't remove this check
      if (value != null) {
        oh.setOption(option, value);
      }
    }
    oh.activateOptions();
  }
