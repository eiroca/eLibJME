...
public class Xml {
  ...
  public static String encode(final String raw) {
    return Xml.encode(raw, Xml.ENCODE_MIN);
  }

  /*
   * encodes an attribute with the given name and value. A single space is
   * inserted before the name of the attribute public static String encodeAttr
   * (String name, String value) { return " "+name+"=\"" + encode (value,
   * ENCODE_QUOT) + "\""; }
   */

  /** encodes a string escaping less than etc. */

  public static String encode(final String raw, final int flags) {

    final int len = raw.length();

    final StringBuffer cooked = new StringBuffer(raw.length());
   ...
   * escaping all quotes and XML special chars contained in the string public
   * static String quote (String q) { return "\"" + encode (q, ENCODE_QUOT) +
   * "\""; }
   */

  /*
   * like String.trim, but additionally all groups of one or more whitespace
   * characters in the string are replaced by a single space (0x040) public
   * static String trim (String s) { char [] buf = new char [s.length ()];
   * boolean ignoring = false; int targetPos = 0; for (int i = 0; i < s.length
   * (); i++) { char c = s.charAt (i); if (c <= ' ') ignoring = true; else { if
   * (ignoring) { if (targetPos != 0) buf [targetPos++] = ' '; ignoring = false; }
   * buf [targetPos++] = c; } } return new String (buf, 0, targetPos); }
   */
}
