...
public class WbxmlWriter extends AbstractXmlWriter {
  ...
  public WbxmlWriter(final OutputStream out) throws IOException {
  ...
  public void checkPending(final boolean degenerated) throws IOException {
    ...
    if (idx == null) {
      ...
      buf.write(len == 0 ? (degenerated ? idx.intValue() : idx.intValue() | 64) : (degenerated ? idx.intValue() | 128 : idx.intValue() | 192));
  ...
  void writeStrT(final String s) throws IOException {
    ...
    if (idx == null) {
      ...
      WbxmlWriter.writeStrI(stringTableBuf, s);
      stringTableBuf.flush();
    }

    WbxmlWriter.writeInt(buf, idx.intValue());
  }
  ...
  public void setTagTable(final int page, final String[] tagTable) {
    ...
    for (int i = 0; i < tagTable.length; i++) {
      if (tagTable[i] != null) {
        final Integer idx = new Integer(i + 5);
  ...
  public void setAttrValueTable(final int page, final String[] attrValueTable) {
    ...
    for (int i = 0; i < attrValueTable.length; i++) {
      if (attrValueTable[i] != null) {
        final Integer idx = new Integer(i + 0x085);
