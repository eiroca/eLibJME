...
/**
 * This class is used to create implementations of XML Pull Parser defined in XMPULL V1 API. The
 * name of actual factory class will be determined based on several parameters. It works similar to
 * JAXP but tailored to work in J2ME environments (no access to system properties or file system) so
 * name of parser class factory to use and its class used for loading (no class loader - on J2ME no
 * access to context class loaders) must be passed explicitly. If no name of parser factory was
 * passed (or is null) it will try to find name by searching in CLASSPATH for
 * org.xmlpull.v1.XmlPullParserFactory resource that should contain a comma separated list of class
 * names of factories or parsers to try (in order from left to the right). If none found, it will
...
public class XmlPullParserFactory {
  ...
  /**
   * Name of the system or midlet property that should be used for a system property containing a
   * comma separated list of factory or parser class names (value:
   * org.xmlpull.v1.XmlPullParserFactory).
   */

  public static final String PROPERTY_NAME = "org.xmlpull.v1.XmlPullParserFactory";
  private static final String RESOURCE_NAME = "/" + XmlPullParserFactory.PROPERTY_NAME;
   ...
   * @return The value of named feature. Unknown features are <string>always</strong> returned as
   *         false
   */
  ...
  /**
   * Specifies that the parser produced by this factory will provide support for XML namespaces. By
   ...
   * XmlPullParser.FEATURE_VALIDATION to true or false). By default the value of this is set to
   * false.
   ...
   * @throws XmlPullParserException if a parser cannot be created which satisfies the requested
   *         configuration.
   */
  ...
  public XmlSerializer newSerializer() throws XmlPullParserException {
    if (serializerClasses == null) { throw new XmlPullParserException("Factory initialization incomplete - has not tried " + classNamesLocation); }
    if (serializerClasses.size() == 0) { throw new XmlPullParserException("No valid serializer classes found in " + classNamesLocation); }
    final StringBuffer issues = new StringBuffer();
  ...
  /**
   * Create a new instance of a PullParserFactory that can be used to create XML pull parsers (see
  ...
  public static XmlPullParserFactory newInstance(String classNames, Class context) throws XmlPullParserException {
    if (context == null) {
      // NOTE: make sure context uses the same class loader as API classes this
      // is the best we can do without having access to context classloader in
      // J2ME if API is in the same classloader as implementation then this will
      // work
    ...
    if ((classNames == null) || (classNames.length() == 0) || "DEFAULT".equals(classNames)) {
      try {
        ...
        if (is == null) { throw new XmlPullParserException("resource not found: " + XmlPullParserFactory.RESOURCE_NAME + " make sure that parser implementing XmlPull API is available"); }
    ...
    while (pos < classNames.length()) {
      ...
      if (candidate != null) {
        ...
        if (!recognized) { throw new XmlPullParserException("incompatible class: " + name); }
      }
      pos = cut + 1;
    }
    if (factory == null) {
