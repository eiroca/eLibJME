...
public class Hierarchy {
   ...
   */
  public Hierarchy(final Category root) {
    ht = new Hashtable();
    this.root = root;
  ...
  public void enable(final Priority p) {
    if (p != null) {
      enableInt = p.level;
      enable = p;
    }
  ...
  /**
   * Returns the string representation of the internal <code>enable</code> state.
   * @since 1.2
   */
   ...
   */
  public Category getInstance(final String name) {
    ...
    synchronized (ht) {
      // LogLog.debug("In getInstance"+dumpht());
  ...
  protected String dumpht() {
    ...
    enu = ht.elements();
    while (enu.hasMoreElements()) {
      final Object o = enu.nextElement();
      rtn += "Values[" + o.getClass().getName() + "; " + o.toString() + "]\n";
    }
    return rtn + "End Dump ht";
  }
}
