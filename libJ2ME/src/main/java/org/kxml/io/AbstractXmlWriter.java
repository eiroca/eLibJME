...
public abstract class AbstractXmlWriter extends Writer {
  ...
  protected State current = new State(null, PrefixMap.DEFAULT, // null, null,
      null);
  ...
  public void startTag(PrefixMap prefixMap, String namespace, final String name) throws IOException {
    ...
    if (prefix == null) {
      // System.out.println ("namespace "+namespace +" not found in
      // "+prefixMap);
    ...
    final String tag = prefix.length() == 0 ? name : prefix + ':' + name;

    final PrefixMap oldMap = current.prefixMap;
    ...
    if (prefixMap != oldMap) {
      PrefixMap current = prefixMap;
      do {
        final String p2 = current.getPrefix();
        final String ns = current.getNamespace();
  ...
  public void startTag(final String namespace, final String name) throws IOException {
    startTag(null, namespace, name);
  }

  /** convenience method for startTag (Xml.NO_NAMESPACE, name) */

  public void startTag(final String name) throws IOException {
