...
public abstract class AbstractXmlParser {
  ...
  public boolean peek(final int type, final String namespace, final String name) throws IOException {

    final ParseEvent pe = peek();
  ...
  public void skip() throws IOException {
    while (true) {
      final int type = peek().type;
      if ((type != Xml.COMMENT) && (type != Xml.DOCTYPE) && (type != Xml.PROCESSING_INSTRUCTION) && (type != Xml.WHITESPACE)) {
        break;
      }
      read();
    }
