...
public class XmlParser extends AbstractXmlParser {
  ...
  public String readName() throws IOException {
    ...
    while (!eof) {
      ...
      if ((c < 128) && (c != '_') && (c != '-') && (c != ':') && (c != '.') && ((c < '0') || (c > '9')) && ((c < 'a') || (c > 'z')) && ((c < 'A') || (c > 'Z'))) {
        break;
      }
  ...
  public XmlParser(final Reader reader, final int bufSize) throws IOException {

    this.reader = reader; // new LookAheadReader (reader);
    buf = new char[bufSize];
  }

  public String resolveCharacterEntity(final String name) throws IOException {
  ...
  ParseEvent parseEndTag() throws IOException {
    ...
    final Tag result = new Tag(Xml.END_TAG, current, current.namespace, current.name);
  ...
  int readText(final StringBuffer buf, final char delimiter) throws IOException {
    ...
    while (true) {
      ...
      if (nextChar == '&') {
        final String code = readTo(';', new StringBuffer()).toString();
        readChar();
